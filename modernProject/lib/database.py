...
class Driver:
    @staticmethod
    def poi(base, i: int):
        from lib.dutil import person_to_gen_person
        if hasattr(base, 'data'):
            persons = base.data.persons.get
            person = persons(i)
            if person:
                strings = base.data.strings.get
                return person_to_gen_person(person, strings, i)
        return None

    @staticmethod
    def foi(base, i: int):
        from lib.dutil import family_to_gen_family
        if hasattr(base, 'data'):
            families = base.data.families.get
            family = families(i)
            if family:
                strings = base.data.strings.get
                return family_to_gen_family(family, strings, i)
        return None
    ...
    def get_father(family):
        if hasattr(family, 'father'):
            return family.father
        return None

    @staticmethod
    def get_mother(family):
        if hasattr(family, 'mother'):
            return family.mother
        return None

    @staticmethod
    def get_consang(person):
        if hasattr(person, 'consang'):
            return person.consang
        from lib import adef
        return adef.no_consang

    @staticmethod
    def ipers(base):
        if hasattr(base, 'data'):
            return list(range(len(base.data.persons.arr)))
        return []
    ...
    def iper_marker(ipers, default_value):
        ...
        if isinstance(ipers, list):
            array = [default_value for _ in range(len(ipers) if ipers else 0)]
        else:
            array = []
